...
class DocumentPipelineService:
    ...
    def _report_progress(self, doc_id: int, stage: str, progress: float, message: str):
        """Report processing progress to global tracker"""
        try:
            ...
            # Don't fail processing if progress reporting fails
    ...
    def process_document(
        ...
        # Capture document info at start for error reporting (survives session rollback)
        doc_id = document.id
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks, 1):
                # Calculate progress: 0.55 to 0.85 (30% of total progress for embeddings)
            ...
            # Refresh document from database to ensure we have the latest state
            # This prevents StaleDataError if the document was modified elsewhere
            try:
                db.refresh(document)
    ...
    def _extract_metadata(self, file_path: str, filename: str) -> Optional[str]:
        """Extract metadata from document's first pages."""
        try:
